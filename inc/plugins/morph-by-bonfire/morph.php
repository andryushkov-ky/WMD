			...
			<div class="morph-main-wrapper-inner">
				<div class="morph-main">
					...
					<div class="morph-heading-wrapper">
						<div class="morph-heading-inner">
							<?php if( is_user_logged_in() ) : global $current_user; ?>
								<a href="<?php echo esc_url( get_author_posts_url( $current_user->data->ID ) ); ?>" class="morph-heading-avatar"><?php echo get_avatar( $current_user->data->ID, 40 ); ?></a>
								<a href="<?php echo esc_url( get_author_posts_url( $current_user->data->ID ) ); ?>" class="morph-heading-text"><?php echo esc_html( $current_user->data->display_name ); ?></a>
								<a href="<?php echo esc_url( get_author_posts_url( 'mailto:' . $current_user->data->user_email ) ); ?>" class="morph-subheading-text"><?php echo esc_html( $current_user->data->user_email ); ?></a>
							<?php else: ?>
								<!-- BEGIN HEADING TEXT -->
								<div class="morph-heading-text">
									<?php echo get_option('bonfire_morph_heading_text'); ?>
								</div>
								<!-- END HEADING TEXT -->
								<!-- BEGIN SUBHEADING TEXT -->
								<?php if( get_option('bonfire_morph_subheading_text') ) { ?>
								<div class="morph-subheading-text">
									<?php echo get_option('bonfire_morph_subheading_text'); ?>
								</div>
								<?php } ?>
								<!-- END SUBHEADING TEXT -->							
							<?php endif; ?>
					...
					<div class="morph-menu-wrapper">						
						<!-- BEGIN MAIN MENU -->						
						<?php 
							$logged_in_menu_item = sprintf( '<li class="has-border-bottom"><a href="%s"><i class="fa fa-home"></i> %s</a></li>', esc_url( home_url( '/' ) ), __( 'Home' ) );

							// Automatically add administration menu item for logged in user
							if( is_user_logged_in() ){
								$logged_in_menu_item .= sprintf( '<li><a href="%s"><i class="fa fa-user"></i> %s</a></li>', esc_url( mdb_current_author_page() ), __( 'Your Profile' ) );
								$logged_in_menu_item .= sprintf( '<li class="has-border-bottom"><a href="%s"> <i class="fa fa-cog"></i>%s</a></li>', esc_url( home_url( '/edit-profile/' ) ), __( 'Edit Profile' ) );
								$logged_in_menu_item .= sprintf( '<li><a href="%s"><i class="fa fa-pencil"></i> %s</a></li>', esc_url( home_url( '/post-new/' ) ), __( 'Submit Article' ) );
								$logged_in_menu_item .= sprintf( '<li class="has-border-bottom"><a href="%s"><i class="fa fa-archive"></i> %s</a></li>', esc_url( home_url( '/posts-list/' ) ), __( 'Your Articles' ) );
							}

							$menu_items_wrap = '<ul id="%1$s" class="%2$s">'. $logged_in_menu_item .'%3$s</ul>';							

							wp_nav_menu( 
								array( 
									'container_class' 	=> 'morph-by-bonfire', 
									'theme_location'	=> 'morph-by-bonfire', 
									'fallback_cb' 		=> '',
